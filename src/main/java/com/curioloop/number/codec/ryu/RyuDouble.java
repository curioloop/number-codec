 ...
 * @see <a href="https://github.com/ulfjack/ryu/blob/master/src/main/java/info/adams/ryu/RyuDouble.java">RyuDouble.java</a>
 */
public final class RyuDouble {
  ...
  private static final char[] CHARS_ZERO = new char[MAX_DOUBLE_LEN+1];
  private static final char[] CHARS_NEG_ZERO = new char[MAX_DOUBLE_LEN+1];
  static {
    CHARS_NAN[MAX_DOUBLE_LEN] = (char) "NaN".length();
    CHARS_POS_INF[MAX_DOUBLE_LEN] = (char) "Infinity".length();
    CHARS_NEG_INF[MAX_DOUBLE_LEN] = (char) "-Infinity".length();
    CHARS_ZERO[MAX_DOUBLE_LEN] = (char) "0.0".length();
    CHARS_NEG_ZERO[MAX_DOUBLE_LEN] = (char) "-0.0".length();
    System.arraycopy("NaN".toCharArray(), 0, CHARS_NAN, 0, CHARS_NAN[MAX_DOUBLE_LEN]);
    System.arraycopy("Infinity".toCharArray(), 0, CHARS_POS_INF, 0, CHARS_POS_INF[MAX_DOUBLE_LEN]);
    System.arraycopy("-Infinity".toCharArray(), 0, CHARS_NEG_INF, 0, CHARS_NEG_INF[MAX_DOUBLE_LEN]);
    System.arraycopy("0.0".toCharArray(), 0, CHARS_ZERO, 0, CHARS_ZERO[MAX_DOUBLE_LEN]);
    System.arraycopy("-0.0".toCharArray(), 0, CHARS_NEG_ZERO, 0, CHARS_NEG_ZERO[MAX_DOUBLE_LEN]);
  }
  ...
  public static void main(String[] args) {
    ...
    System.out.println(RyuFormatter.decimal(value) + " " + value);
  ...
  static Object doubleTo(double value, RoundingMode roundingMode, char[] result) {
    ...
    if (value == Double.POSITIVE_INFINITY) return result == null ? null : CHARS_POS_INF;
    if (value == Double.NEGATIVE_INFINITY) return result == null ? null : CHARS_NEG_INF;
    long bits = Double.doubleToLongBits(value);
    if (bits == 0) return result == null ? BigDecimal.valueOf(0, 1) : CHARS_ZERO;
    if (bits == 0x8000000000000000L) return result == null ? new BigDecimal("-0.0") : CHARS_NEG_ZERO;
    ...
    if (result.length < MAX_DOUBLE_LEN + 1)
      throw new IllegalStateException();
    ...
    if (scientificNotation) {
      ...
      result[index++] = (char) ('0' + exp % 10);
      result[MAX_DOUBLE_LEN] = (char) index;
    } else {
