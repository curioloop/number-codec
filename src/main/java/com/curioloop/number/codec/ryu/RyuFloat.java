 ...
 * @see <a href="https://github.com/ulfjack/ryu/blob/master/src/main/java/info/adams/ryu/RyuFloat.java">RyuFloat.java</a>
 */
public final class RyuFloat {
  private static boolean DEBUG = false;

  public static final int MAX_FLOAT_LEN = 15;
  private static final char[] CHARS_NAN = new char[MAX_FLOAT_LEN+1];
  private static final char[] CHARS_POS_INF = new char[MAX_FLOAT_LEN+1];
  private static final char[] CHARS_NEG_INF = new char[MAX_FLOAT_LEN+1];
  private static final char[] CHARS_ZERO = new char[MAX_FLOAT_LEN+1];
  private static final char[] CHARS_NEG_ZERO = new char[MAX_FLOAT_LEN+1];
  static {
    CHARS_NAN[MAX_FLOAT_LEN] = (char) "NaN".length();
    CHARS_POS_INF[MAX_FLOAT_LEN] = (char) "Infinity".length();
    CHARS_NEG_INF[MAX_FLOAT_LEN] = (char) "-Infinity".length();
    CHARS_ZERO[MAX_FLOAT_LEN] = (char) "0.0".length();
    CHARS_NEG_ZERO[MAX_FLOAT_LEN] = (char) "-0.0".length();
    System.arraycopy("NaN".toCharArray(), 0, CHARS_NAN, 0, CHARS_NAN[MAX_FLOAT_LEN]);
    System.arraycopy("Infinity".toCharArray(), 0, CHARS_POS_INF, 0, CHARS_POS_INF[MAX_FLOAT_LEN]);
    System.arraycopy("-Infinity".toCharArray(), 0, CHARS_NEG_INF, 0, CHARS_NEG_INF[MAX_FLOAT_LEN]);
    System.arraycopy("0.0".toCharArray(), 0, CHARS_ZERO, 0, CHARS_ZERO[MAX_FLOAT_LEN]);
    System.arraycopy("-0.0".toCharArray(), 0, CHARS_NEG_ZERO, 0, CHARS_NEG_ZERO[MAX_FLOAT_LEN]);
  }
  ...
  public static void main(String[] args) {
    DEBUG = true;
    float f = -0.33007812f;
    System.out.println(RyuFormatter.decimal(f) + " " + f);
    System.out.println(RyuFormatter.string(f) + " " + f);
  }

  public static Object floatTo(float value, RoundingMode roundingMode, char[] result) {
    // Step 1: Decode the floating point number, and unify normalized and subnormal cases.
    // First, handle all the trivial cases.
    if (Float.isNaN(value)) return result == null ? null : CHARS_NAN;
    if (value == Float.POSITIVE_INFINITY) return result == null ? null : CHARS_POS_INF;
    if (value == Float.NEGATIVE_INFINITY) return result == null ? null : CHARS_NEG_INF;
    int bits = Float.floatToIntBits(value);
    if (bits == 0) return result == null ?  BigDecimal.valueOf(0, 1) : CHARS_ZERO;
    if (bits == 0x80000000) return result == null ?  new BigDecimal("-0.0") : CHARS_NEG_ZERO;
    ...
    if (result == null) {
      ...
      return BigDecimal.valueOf(output, -(exp - olength + 1));
    }

    // Step 5: Print the decimal representation.
    // We follow Float.toString semantics here.
    if (result.length < MAX_FLOAT_LEN + 1)
    ...
    result[MAX_FLOAT_LEN] = (char) index;
    return result;
  }
